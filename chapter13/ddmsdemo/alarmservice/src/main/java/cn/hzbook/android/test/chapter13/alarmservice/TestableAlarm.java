...
public class TestableAlarm {
	...
	public IGetDate getIGetDate() { return _getDate; }
	public void setIGetDate(IGetDate getDate) { _getDate = getDate; }
	
	public TestableAlarm(Option option, IGetDate getdate, Context context) {
		_timer = new Timer();
		_option = option;
		_getDate = getdate;
		_context = context;
	...
	public void start() {
		TimerTask task = new TimerTask() {
			...
			private void printAlarm(Date now) {
				trigerTimes++;
				String text = String.format("闹铃时间：%1$s，重复设置：%2$s，当前时间：%3$s",
